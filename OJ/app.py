...
================================================================================
文件结构导航（Streamlit 脚本自上而下执行；侧边栏导航与下面各页面函数一一对应）
================================================================================
  0. 基础配置           后端地址、会话 Cookie 名、页面全局设置
  1. 统一 API 调用      is_logged_in / current_user / api()，集中处理请求与身份
  2. 通用小工具          题目列表、标签拆分、样例/测试点编辑器等复用组件
  3. 用户页面组          登录/注册、我的信息、用户管理（页面「用户中心」）
  4. 题目页面组          列表/新增/编辑/删除（页面「题目管理」）
  5. 评测与提交页面组    代码提交、提交记录、提交详情（页面「评测与提交」）
  6. 语言管理            查询 + 动态注册语言（页面「语言管理」）
  7. AI 智能命题         模型配置 + 智能生成题目（页面「AI 智能命题」）
  8. 入口 main()         侧边栏登录态 + 导航路由，按选中项调用对应页面函数

关键约定
- 前端只通过 REST API 调用后端，不直接读写后端数据文件
- 会话基于后端下发的 Cookie（session id），前端保存 token 并在每次请求携带
- 所有响应遵循后端 {code, msg, data} 结构，HTTP 状态码与 code 一致
- 后端不可达时 api() 返回 {code:-1}，由 _msg() 统一给出友好提示
