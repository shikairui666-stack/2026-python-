...
def song_list(request):
    ...
    paginator=Paginator(songs, 15)#此处调试以后发现15方便
...
def artist_list(request):
    ...
    paginator = Paginator(artists, 15)#同理
    page_number = request.GET.get('page', 1)
    page_obj = paginator.get_page(page_number)
...
def artist_detail(request, artist_id):
    ...
    songs = artist.songs.order_by('id')  # 靠 related_name='songs' 反向查到这个歌手的所有歌
...
def search(request):
    ...
    page_obj = Paginator(results, 15).get_page(request.GET.get('page', 1))
